...
def eps(i) :
    res = 0
    for j in train_wrong_classified_lists[i] :
        res+=lambda_list_train[j]
    return float(res)
...
for k in range(N, N+num_epochs) :
    ...
    alpha_list_train.append([i_k, alpha_k ] )
    
    normalized = float(epsilon_i_k * (np.exp( alpha_k) - np.exp(- alpha_k)) + np.exp(- alpha_k))
    for j in range(len_train_dataset) :
        if(j in train_wrong_classified_lists[i_k]) :
            lambda_list_train[j] *= np.exp( alpha_k) / normalized
        else :
            lambda_list_train[j] *= np.exp(- alpha_k) / normalized
N+=num_epochs
print("---- done ----")
